...
Polymarket WebSocket (CLOB)   ──┘        │                         │
                     ...
                     (velocity + recency)                          │
                              │                                    │
                              ▼                                    │
                     AI Explanation Builder ──▶ Groq Llama 3.3 70B │
                     (template fallback)                           │
                              │                                    │
                              ▼                                    │
                     market_movements ◀────────────────────────────┘
...
| `services/explanations` | AI-powered explanation generation (Groq Llama 3.3 70B) with template fallback |
| `services/news` | Fetches news coverage scores from NewsAPI.org with AI-generated search keywords |
...
**Files:** `services/explanations/src/ai.client.ts`, `services/explanations/src/buildExplanation.ts`

### AI Explanations (Primary)

When `GROQ_API_KEY` is set, explanations are generated by Llama 3.3 70B (via Groq) acting as a quantitative analyst. The model receives a structured context block containing all movement metrics, signal scores, classification, and news headlines:

```
MARKET: {title}
OUTCOME: {outcome}  |  WINDOW: {window_type} ({duration})

PRICE: 0.4200 → 0.4800 (Δ 14.3%), range 16.1%
VOLUME: 12450 (ratio: 2.31x baseline, hourly peak: 3.12x)
VELOCITY: 0.0134  |  REASON: VELOCITY
LIQUIDITY: thin=false, trades=87, levels=14

SCORES: capital=0.69 info=0.12 velocity=0.67 news=0.42 time=0.15
CLASSIFICATION: VELOCITY (confidence: 0.72)

NEWS: "Congress passes spending bill" | "Government shutdown averted"
```

The system prompt instructs the model to:
- Lead with the most likely driver of the move
- Reference specific numbers (price %, volume ratios, velocity)
- Cite relevant news headlines when available
- Note thin liquidity when applicable
- Output 2-3 sentences max, plain English, no formatting

**Config:** `temperature: 0.3`, `max_tokens: 150`, 8s timeout via `Promise.race`

### Template Explanations (Fallback)

When `GROQ_API_KEY` is not set, or on any AI failure (timeout, rate limit, empty response), the system falls back to template-based explanations that compose sentences from:

1. **Price sentence:** "Price moved X% over Y min/hours."
2. **Window sentence:** Window-specific context
3. **Volume sentence:** Based on volume ratio
4. **Liquidity sentence:** Thin orderbook warning (if applicable)
5. **Classification sentence:** Human-readable driver explanation
6. **Velocity sentence:** Speed context (if reason is VELOCITY)
7. **News sentence:** Related headline excerpts (if available)

No AI failure can crash the detection pipeline — fallback is always synchronous and deterministic.

### News Keyword Generation

**File:** `services/news/src/ai.keyword.ts`

When `GROQ_API_KEY` is set, the news service uses Llama 3.3 70B (via Groq) to generate smarter search queries from market titles before querying NewsAPI.org. For example, a market titled "Will there be a government shutdown?" generates:

```
"government shutdown" OR "spending bill" OR Congress budget OR "continuing resolution"
```

This replaces the basic stop-word filtering approach which often missed relevant articles. AI keywords are cached hourly per slug. On failure, falls back to the original `buildSearchQuery` stop-word method.
...
| Variable | Default | Description |
|---|---|---|
| `GROQ_API_KEY` | (none) | Required for AI explanations and news keywords; free at console.groq.com/keys |
| `AI_EXPLANATION_MODEL` | `llama-3.3-70b-versatile` | Model for explanations and news keywords |
| `AI_BASE_URL` | `https://api.groq.com/openai/v1` | Override to use a different OpenAI-compatible provider |
| `AI_EXPLANATION_TIMEOUT_MS` | 8000 | Max wait before template fallback |
...
# Groq (optional — enables AI explanations and smarter news search)
GROQ_API_KEY=gsk_your-key-here              # Free at console.groq.com/keys
