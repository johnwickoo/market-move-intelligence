...
Polymarket WebSocket (CLOB)   ──┘        │                         │
                     ...
                     (velocity + recency)                          │
                              │                                    │
                              ▼                                    │
                     AI Explanation Builder ──▶ OpenAI GPT-4o-mini │
                     (template fallback)                           │
                              │                                    │
                              ▼                                    │
                     market_movements ◀────────────────────────────┘
...
| `services/explanations` | AI-powered explanation generation (OpenAI GPT-4o-mini) with template fallback |
...
When `OPENAI_API_KEY` is set, explanations are generated by GPT-4o-mini acting as a quantitative analyst. The model receives a structured context block containing all movement metrics, signal scores, classification, and news headlines:
...
When `OPENAI_API_KEY` is not set, or on any AI failure (timeout, rate limit, empty response), the system falls back to template-based explanations that compose sentences from:
...
When `OPENAI_API_KEY` is set, the news service uses GPT-4o-mini to generate smarter search queries from market titles before querying NewsAPI.org. For example, a market titled "Will there be a government shutdown?" generates:
...
| Variable | Default | Description |
|---|---|---|
| `OPENAI_API_KEY` | (none) | Required for AI explanations and news keywords; if unset, template fallback |
| `AI_EXPLANATION_MODEL` | `gpt-4o-mini` | Model for explanations and news keywords |
...
# OpenAI (optional — enables AI explanations and smarter news search)
